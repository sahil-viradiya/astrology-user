...
class _LiveScreenState extends State<LiveScreen> {
  ...
  onInit() async {
    ...
    await createClient(); // comment this if firebase live chat enable
    getWaitList();
  }
  ...
  Future<void> createClient() async {
    try {
      ...
      client.addListener(
        message: (event) async {
          try {
            log('Message event ${event.publisher}');
        ...
        presence: (event) {
          ...
          // myviewerCount.value = event.interval!.joinUserList!.users!.length;
        },
        token: (event) {
                                    ...
                                    : SizedBox(
                                        ...
                                        child: ListView.builder(
                                          ...
                                          itemBuilder: (context, index) {
                                            debugPrint(
                                              'revrese profile ${messages[index].profile}',
                                            );
                                                                ...
                                                                messages[index].gift !=
                                                                    ...
                                                                    ? CachedNetworkImage(
                                                                        ...
                                                                        width:
                                                                            30,
                                                                        imageUrl:
                                                                            '$imgBaseurl${messages[index].gift}',
                                                                      )
                                                                    : const SizedBox(),
                                                              ],
                                                            ),
  ...
  void dispose() async {
    ...
    // 3. ---------Firebase clear data for fututre-------------
    // try {
    //   await chatcontroller.deleteBatches(channel_name, true);
    // } on Exception catch (e) {
    //   debugPrint('Exception in deleteBatches: ${e.toString()}');
    // }
    // --------------------------------------------------
